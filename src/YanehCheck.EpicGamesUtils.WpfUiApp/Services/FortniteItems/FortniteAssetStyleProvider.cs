...
public class FortniteAssetStyleProvider(IFortniteAssetSerializer assetSerializer, IItemStyleAssetMapper mapper) : IFortniteAssetStyleProvider {
    public async Task<ConcurrentBag<ItemStyleModel>> GetFromPackagePropertiesFileRecursive(string directory, Action<double>? progressReport = null) {
    ...
    public async Task<List<ItemStyleModel>?> GetFromSerializedJson(string file, Action<double>? progressReport = null) {
